...
- `identity-pilot.sqlite`：实验性统一 QQ 身份索引，仅在启用实验开关后创建；
- `slang-pilot.sqlite`：黑话发现、研究任务与两级审批审计，仅在启用后创建；
...
「设置 -> 人设 -> 选择人设」提供多张内置角色卡（默认小鲸鱼、游戏客户端开发者、损友、温柔陪聊、
技术宅、猫娘）。选择模板仅填入草稿，点击“保存人设修改”后生效。交流策略可选“原版群友”或“自然可靠”，
角色正文与管理员附加规则均可修改，也可基于当前草稿通过“＋ 添加人设”创建自定义副本。角色卡正文的
单一来源是 [`roles/`](roles) 目录，一张卡对应一个 markdown 文件。
...
`person_memory_lookup` 查询工具。「好友管理」页维护主动候选、收到的好友请求、审批参数及状态。
管理员批准主动候选后，系统通过 SnowLuma `send_packet` 调用已验证的 QQ 好友协议；仅业务响应明确
成功时标记为已提交，超时、断线或响应无法解析时进入“发送结果未知”状态且禁止自动重试。收到
`friend_add` 事件后才闭环为“已成为好友”。收到的好友请求同样需要管理员审批，同意后调用标准
`set_friend_add_request` 并自动加入私聊白名单。实现边界见[已转正的稳定特性](docs/STABLE_FEATURES.md)。
...
CI（GitHub Actions）在每次推送和 PR 上执行语法检查、未定义调用扫描（严格模式）、单元测试与本地回归。
